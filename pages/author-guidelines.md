...
The criteria for reproducibility that will be assessed during the reviewing process are as follows.
 ...
 - The materials should satisfy the minimum expectations laid out in the [reproducibility checklist](pages/author-checklist.html), which, as of August 2026, authors are expected to have reviewed before submitting their materials.
 ...
 - We encourage authors to deposit artifacts, such as code and data, that support the results in the manuscript in suitable open repositories (ideally permanent repositories that provide DOIs). While we encourage using public Git repositories for code (but not large datasets) (such as on GitHub, GitLab, and BitBucket), this is not sufficient for long-term access, since repositories can be deleted or made private. Given that, if materials are only available in Git repositories, they should also be included as supplementary materials. Code and data (<500 MB) in supplementary materials will be deposited upon acceptance in the [JASA area of Figshare](https://tandf.figshare.com/JASA). 
 - The ACC Form should indicate where the code and data will be made publicly available upon publication of the manuscript, but code and data need not be publicly available at the time of submission when there are concerns about confidentiality. As with the contents of the manuscript, code and data will be considered to be confidential unless they are publicly available at the time of review. 
 - Authors may choose to anonymize their materials by removing identifying information in the submitted supplemental materials or public repository, or via an anonymous repository such as through [Anonymous GitHub](https://anonymous.4open.science/). 
 ...
 - Once a manuscript is conditionally accepted, the code and data (unless an exception is granted for the latter) must be made available as detailed in the ACC form and any anonymous GitHub repository should be replaced by a public non-anonymous Git repository. Manuscripts will not be accepted and should not be claimed as such by the authors until this occurs.
