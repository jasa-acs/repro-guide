...
3. Authors must review the [reproducibility checklist](pages/author-checklist.html) ([PDF version](pages/author-checklist.pdf)) before submitting their reproducibility materials with their revision.
3. We strongly encourage the use of a Git repository for code and workflow materials (but not large datasets) hosted on a site such as GitHub, GitLab, or BitBucket. If desired, authors might choose to keep the repository private until the time of publication. We provide an optional [template repository](https://github.com/jasa-acs/repro-template) as an example on which you can base your repository. 
4. Your submitted supplementary materials should include:
  - the [PDF of the ACC form](pages/acc.html) as a separate file,
  - any supplementary material files not directly related to reproducibility (e.g., additional figures/tables, proofs), and
  - a single zip file (with the suggested name of `reproducibility_materials.zip`) of your reproducibility materials, including any materials in a GitHub (or the like) repository (so as to ensure long-term availability).
5. Feel free to look at previously published articles for example reproducible materials. Below are several recent examples of articles that meet or exceed the reproducibility standards:
